...
class SystemHealthMonitor {
    ...
    async checkDatabaseHealth() {
        ...
        try {
            ...
            await this.db.run('INSERT OR REPLACE INTO system_health (component, status, last_check, details) VALUES (?, ?, ?, ?)', ['DatabaseHealthCheck', 'Healthy', new Date().toISOString(), 'Health check query successful']);
     ...
     */
    async checkDataSourceConnections() {
        const results = [];
        try {
     ...
     */
    async processHealthAlerts(results) {
        if (!this.alertService) {
     ...
     */
    async sendSystemHealthAlert(component, severity) {
        try {
            const alertMessage = {
                jobId: 'system-health',
     ...
     */
    async storeHealthStatus(status) {
        try {
            const sql = `
     ...
     */
    async loadHealthStatusesFromDatabase() {
        try {
