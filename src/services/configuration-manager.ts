...
export class ConfigurationManagerService implements ConfigurationManager {
   ...
   */
  async addJobConfiguration(config: JobConfiguration): Promise<void> {
    ...
    try {
      ...
      const validationResult = ConfigurationValidator.validateJobConfiguration(config);
   ...
   */
  async updateJobConfiguration(jobId: string, config: Partial<JobConfiguration>): Promise<void> {
    this.logger.info('ConfigurationManager', `Updating job configuration: ${jobId}`);

    try {
      // Validate jobId
      if (!jobId || typeof jobId !== 'string' || jobId.trim().length === 0) {
        throw new ValidationError('Job ID is required and must be a non-empty string');
      }
      ...
      const mergedConfig: JobConfiguration = {
        ...existingConfig,
        ...config,
      ...
      const validationResult = ConfigurationValidator.validateJobConfiguration(mergedConfig);
      if (!validationResult.isValid) {
        const errorMessage = `Configuration validation failed: ${validationResult.errors.map(e => e.message).join(', ')}`;
      ...
      throw error instanceof ValidationError ? error : new Error(errorMessage);
    }
  }
   ...
   */
  async removeJobConfiguration(jobId: string): Promise<void> {
   ...
   */
  async getJobConfiguration(jobId: string): Promise<JobConfiguration> {
   ...
   */
  async getAllJobConfigurations(): Promise<JobConfiguration[]> {
    ...
    try {
      ...
      const configurations: JobConfiguration[] = [];
   ...
   */
  async validateConfiguration(config: JobConfiguration): Promise<boolean> {
   ...
   */
  private async getJobConfigurationInternal(jobId: string): Promise<JobConfiguration | undefined> {
    const sql = `
      SELECT job_id, name, schedule, expected_duration, log_paths, 
             data_source, alert_channels, sla_thresholds, created_at, updated_at
      FROM job_configurations 
    ...
    const row = await this.db.get<any>(sql, [jobId]);
    if (!row) {
      return undefined;
    }
   ...
   */
  private parseConfigurationRow(row: any): JobConfiguration {
    try {
      return {
        jobId: row.job_id,
        name: row.name,
